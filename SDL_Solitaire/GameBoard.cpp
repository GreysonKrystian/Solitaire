...
Board::Board()
{
	auto tile_one = new Tile({100, 250});
	auto tile_two = new Tile({100 + 220, 250});
	auto tile_three = new Tile({100 + 220 * 2, 250});
	auto tile_four = new Tile({100 + 220 * 3, 250});
	auto tile_five = new Tile({100 + 220 * 4, 250});
	auto tile_six = new Tile({100 + 220 * 5, 250});
	auto tile_seven = new Tile({100 + 220 * 6, 250});

	auto pile_one = new Pile({600, 20});
	auto pile_two = new Pile({850, 20});
	auto pile_three = new Pile({1100, 20});
	auto pile_four = new Pile({1350, 20});
...
void Board::setRevealedCards() // syf
{
	for (auto itr = tiles.begin(); itr != tiles.end(); ++itr)
	{
		for (auto iter = (*itr)->getCardsOnTile().begin(); iter != (*itr)->getCardsOnTile().end() && (*iter) != nullptr;
		     ++iter)
		{
			if ((*iter)->isOnTop())
			{
...
void Board::putCardFromDeckOnTile(Tile* tile, Card* chosen_card)
{
	if (!tile->getCardsOnTile().empty())
	{
		...
		chosen_card->setPosition({
			                         tile->getPositionOnBoard().x,
			                         tile->getCardsOnTile().back()->getRevealedPartPosition()[0].y + 20
		                         },
		                         {
			                         tile->getPositionOnBoard().x + chosen_card->getSize().x,
			                         tile->getCardsOnTile().back()->getRevealedPartPosition()[0].y + chosen_card->
			                         getSize().y + 20
		                         });
	}
	else
	{
		chosen_card->setPosition({tile->getPositionOnBoard().x, tile->getPositionOnBoard().y}, {
			                         tile->getPositionOnBoard().x + chosen_card->getSize().x,
			                         tile->getPositionOnBoard().y + chosen_card->getSize().y
		                         });
	}
...
void Board::changeTileOfCards(Tile* old_tile, Tile* new_tile, Card* chosen_card)
{
	...
	for (auto itr = cards_to_transfer.begin(); itr != cards_to_transfer.end(); ++itr)
	{
		if (new_tile->getCardsOnTile().empty())
		{
			(*itr)->setPosition({new_tile->getPositionOnBoard().x, new_tile->getPositionOnBoard().y}, {
				                    new_tile->getPositionOnBoard().x + chosen_card->getSize().x,
				                    new_tile->getPositionOnBoard().y + chosen_card->getSize().y
			                    });
		}
		else
		{
			(*itr)->setPosition({
				                    new_tile->getPositionOnBoard().x,
				                    new_tile->getCardsOnTile().back()->getRevealedPartPosition()[0].y + 20
			                    },
			                    {
				                    new_tile->getPositionOnBoard().x + chosen_card->getSize().x,
				                    new_tile->getCardsOnTile().back()->getRevealedPartPosition()[0].y + chosen_card->
				                    getSize().y + 20
			                    });
		}
...
bool Board::isPlacingCardHereLegal(Tile* new_tile, Card* clicked_card)
{
	...
	if (!new_tile->getCardsOnTile().empty())
	{
		...
		{
			if (new_tile->getCardsOnTile().back()->getColor() == "spades" || new_tile->getCardsOnTile().back()->
				getColor() == "clubs")
			{
				return false;
			}
		...
		{
			if (new_tile->getCardsOnTile().back()->getColor() == "diamonds" || new_tile->getCardsOnTile().back()->
				getColor() == "hearts")
			{
				return false;
			}
