...
void Game::updateCards(sf::RenderWindow& window, std::vector<Card*> currently_moved_cards)
{
	for (auto itr = board.getTiles().begin(); itr != board.getTiles().end(); ++itr)
	{
		for (auto iter = (*itr)->getCardsOnTile().begin(); iter != (*itr)->getCardsOnTile().end(); ++iter)
		{
			if (!(std::find(currently_moved_cards.begin(), currently_moved_cards.end(), (*iter)) !=
				currently_moved_cards.end()))
			{
				(*iter)->drawCard(window, false);
			}
		}
...
bool Game::checkIfReleasedCardsInArea(float mouse_pos_x, float mouse_pos_y)
{
	...
	for (unsigned int i = 0; i < top_cards.size(); i++)
	{
		if ((*itr) != nullptr)
		{
			if (mouse_pos_x >= (*itr)->getRevealedPartPosition()[0].x && mouse_pos_x <= (*itr)->
				getRevealedPartPosition()[1].x &&
				mouse_pos_y >= (*itr)->getRevealedPartPosition()[0].y && mouse_pos_y <= (*itr)->
				getRevealedPartPosition()[1].y)
			{
				return true;
			}
		}
...
bool Game::isCardInPilesArea(sf::RenderWindow& window)
{
	for (auto itr = board.getPiles().begin(); itr != board.getPiles().end(); ++itr)
	{
		if (sf::Mouse::getPosition(window).x > (*itr)->getPosition().x && sf::Mouse::getPosition(window).x < (*itr)->
			getPosition().x + getCardsSize().x &&
			sf::Mouse::getPosition(window).y > (*itr)->getPosition().y && sf::Mouse::getPosition(window).y < (*itr)->
			getPosition().y + getCardsSize().y)
			return true;
	}
	return false;
}
